...
from utils import FocalDiceloss_IoULoss, get_logger, generate_point, setting_prompt_none,SurfaceLoss,BoundaryFocalDiceloss_IoULoss
...
def parse_args():
    ...
    parser.add_argument("--run_name", type=str, default=f"sam-med2d_none_ada_{choice}_boundary", help="run model name")
    ...
    parser.add_argument("--data_path", type=str, default="", help="train data path") 
    parser.add_argument("--metrics", nargs='+', default=['dice'], help="metrics")
    ...
    parser.add_argument("--point_list", type=list, default=[3,4,5,6,7,8,9,10], help="point_list")
...
def main(args):
    ...
    criterion = BoundaryFocalDiceloss_IoULoss(weight=1.0)
    # criterion = FocalDiceloss_IoULoss()
    if args.lr_scheduler:
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[10,20, 30,45,60,75,90], gamma = 0.5)
        # scheduler = torch.optim.lr_scheduler.OneCycleLR(
        #     optimizer,
        #     max_lr=args.lr,
        #     epochs=args.epochs,
        #     pct_start=0.05,
        #     steps_per_epoch=int(1441/args.batch_size)*(args.iter_point+1),
    ...
    train_dataset = TrainingDataset(args.data_path, image_size=args.image_size, mode='train', point_num=10, mask_num=args.mask_num, requires_name = False,choice=choice)
    ...
    for epoch in range(0, args.epochs):
        ...
        dice=float(train_metrics['dice'])
        if dice > best_dice:
            best_dice = dice
            if best_dice>0.98:
                save_path = os.path.join(args.work_dir, "models", args.run_name, f"epoch{epoch+1}_sam_{dice}.pth")
